...
export class NewsEditComponent implements OnInit {
  ...
  readProduct() {
    this.serviceProviderService.post('m/product/read', { permission: this.permission }).subscribe(data => {
      let model: any = {};
      model = data;
      this.productList = [{ value: '', display: 'งานภายใน' }];
      model.objectData.forEach(element => {
        this.productList.push({ value: element.code, display: element.title });
      });
      if (this.code == '')
  ...
  galleryFileRead() {
    this.serviceProviderService.post('m/news/galleryFile/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  exportAsXLSX(): void {
    this.serviceProviderService.post('news/comment/read', { skip: 0, limit: 999999, code: this.code }).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
