...
export class UserRoleListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('userRolePage') != null) {
      this.permission = JSON.parse(localStorage.getItem('userRolePage'));
    }
  }
  ...
  create() {
    this.router.navigate(['user-role-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['user-role-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('register/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('register/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
