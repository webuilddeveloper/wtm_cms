...
@Component({
  selector: 'app-certificate-category-list',
  templateUrl: './certificate-category-list.component.html',
  styleUrls: ['./certificate-category-list.component.css']
})
export class CertificateCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('certificateCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('certificateCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['certificate-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['certificate-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('certificate/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('certificate/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
