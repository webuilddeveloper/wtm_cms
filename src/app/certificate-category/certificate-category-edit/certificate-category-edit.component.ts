...
@Component({
  selector: 'app-certificate-category-edit',
  templateUrl: './certificate-category-edit.component.html',
  styleUrls: ['./certificate-category-edit.component.css']
})
export class CertificateCategoryEditComponent implements OnInit {

  editModel: any = {};
  code: any;
  title = 'แก้ไขข้อมูลหมวดหมู่พนักงาน';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลหมวดหมู่พนักงาน';
        this.read();
      }
    });
  ...
  create() {
    ...
    this.serviceProviderService.post('certificate/category/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('certificate/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    // if (this.editModel.image != undefined)
    //   this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('certificate/category/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['certificate-category'], { skipLocationChange: true });
  }
