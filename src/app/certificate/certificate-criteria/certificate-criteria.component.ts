...
@Component({
  selector: 'app-certificate-criteria',
  templateUrl: './certificate-criteria.component.html',
  styleUrls: ['./certificate-criteria.component.css']
})
export class CertificateCriteriaComponent implements OnInit {
  ...
  readSequence() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'certificate' }).subscribe(data => {
      let model: any = {};
      model = data;
