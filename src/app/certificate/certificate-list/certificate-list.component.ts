...
export class CertificateListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('certificatePage') != null) {
      let model: any = [];
      this.permission = JSON.parse(localStorage.getItem('certificatePage'));
  ...
  create() {
    this.router.navigate(['certificate-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['certificate-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('certificate/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('certificate/reorder', { code: model }).subscribe(data => {
  ...
  reOrderAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('certificate/reorderAll', { code: model }).subscribe(data => {
          this.toastr.success('Re Order เรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          }, 2000)
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('certificate/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          }, 2000)
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('certificate/report/read', obj).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
