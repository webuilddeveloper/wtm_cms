...
export class AboutCommentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('aboutCommentPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('aboutCommentPage'));
  ...
  read() {
    ...
    if (this.isAdvanceSearch)
      ...
      this.serviceProviderService.post('AboutUs/readComment', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
