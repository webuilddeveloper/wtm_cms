...
@Component({
  selector: 'app-office-activities-list',
  templateUrl: './office-activities-list.component.html',
  styleUrls: ['./office-activities-list.component.css']
})
export class OfficeActivitiesListComponent implements OnInit {
  ...
  create() {
    this.router.navigate(['office-activities-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['office-activities-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('OfficeActivities/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('OfficeActivities/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
