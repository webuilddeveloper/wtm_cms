...
@Component({
  selector: 'app-office-activities',
  templateUrl: './office-activities.component.html',
  styleUrls: ['./office-activities.component.css']
})
export class OfficeActivitiesComponent implements OnInit {
  model: any = [];
  listModel: string = '';
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('OfficeActivitiesPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('OfficeActivitiesPage'));
  ...
  read() {
    ...
    this.serviceProviderService.post('OfficeActivities/read', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
