...
@Component({
  ...
  styleUrls: ['./work-process-list.component.css']
})
export class WorkProcessListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('workProcessPage') != null) {
      let model: any = [];
      this.permission = JSON.parse(localStorage.getItem('workProcessPage'));
  ...
  create() {
    this.router.navigate(['work-process-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['work-process-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('workprocess/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('workprocess/reorder', { code: model }).subscribe(data => {
  ...
  reOrderAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('workprocess/reorderAll', { code: model }).subscribe(data => {
          this.toastr.success('Re Order เรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          }, 2000)
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('workprocess/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          }, 2000)
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('workprocess/report/read', obj).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
