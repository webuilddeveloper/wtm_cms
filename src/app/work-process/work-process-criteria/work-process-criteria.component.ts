...
@Component({
  selector: 'app-work-process-criteria',
  templateUrl: './work-process-criteria.component.html',
  styleUrls: ['./work-process-criteria.component.css']
})
export class WorkProcessCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('workProcessPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('workProcessPage'));
  ...
  readSequence() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'workProcess' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('workprocess/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
