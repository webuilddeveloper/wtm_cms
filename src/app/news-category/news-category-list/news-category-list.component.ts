...
@Component({
  ...
  styleUrls: ['./news-category-list.component.css']
})
export class NewsCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('newsCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('newsCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['news-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['news-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('news/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('news/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
