...
@Component({
  selector: 'app-alliance-edit',
  templateUrl: './alliance-edit.component.html',
  styleUrls: ['./alliance-edit.component.css']
})
export class AllianceEditComponent implements OnInit {

  title = 'เพิ่มข้อมูลพันธมิตร';
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.read();
        this.title = 'แก้ไขข้อมูลพันธมิตร';
      }
  ...
  create() {
    ...
    this.serviceProviderService.post('alliance/create', this.editModel).subscribe(data => {
      ...
      if (this.editModel.gallery.length > 0) {
        this.editModel.gallery.forEach(element => {
          ...
          this.serviceProviderService.post('alliance/gallery/create', element).subscribe(data => { }, err => { });
        });
      }
  ...
  read() {
    this.serviceProviderService.post('alliance/read', { code: this.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    this.serviceProviderService.post('alliance/update', this.editModel).subscribe(data => {

      this.serviceProviderService.post('alliance/gallery/delete', this.editModel).subscribe(data => {
        if (this.editModel.gallery.length > 0) {
          this.editModel.gallery.forEach(element => {
  ...
  galleryRead() {
    this.serviceProviderService.post('m/alliance/gallery/read', { code: this.editModel.code }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  back() {
    this.router.navigate(['alliance'], { skipLocationChange: true });
  }
