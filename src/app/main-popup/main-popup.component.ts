...
export class MainPopupComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('mainPopupPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('mainPopupPage'));
  ...
  read() {
    ...
    this.serviceProviderService.post('mainpopup/read', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
