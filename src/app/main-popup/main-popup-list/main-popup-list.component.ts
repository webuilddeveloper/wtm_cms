...
@Component({
  selector: 'app-main-popup-list',
  templateUrl: './main-popup-list.component.html',
  styleUrls: ['./main-popup-list.component.css']
})
export class MainPopupListComponent implements OnInit {
  ...
  create() {
    this.router.navigate(['main-popup-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['main-popup-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('mainpopup/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('mainpopup/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
