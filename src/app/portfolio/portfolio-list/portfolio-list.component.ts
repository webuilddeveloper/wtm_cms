...
export class PortfolioListComponent implements OnInit {
  ...
  @Input() messageInput: any = [];
    ...
    create() {
      this.router.navigate(['portfolio-edit', ''], { skipLocationChange: true });
    }
  
    view(param) {
      this.router.navigate(['portfolio-edit', param], { skipLocationChange: true });
    }
    ...
    delete(param, idx) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        if (result) {
          this.serviceProviderService.post('portfolio/delete', { code: param.code }).subscribe(data => {
    ...
    reOrder() {
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        if (result) {
          ...
          this.serviceProviderService.post('portfolio/reorder', { code: model }).subscribe(data => {
    ...
    reOrderAll() {
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        if (result) {
          this.serviceProviderService.post('portfolio/reorderAll', { code: model }).subscribe(data => {
            this.toastr.success('Re Order เรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
            setTimeout(() => {
              window.location.reload()
            }, 2000)
    ...
    deleteAll() {
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        if (result) {
          ...
          this.serviceProviderService.post('portfolio/delete', { code: model }).subscribe(data => {
            this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
            setTimeout(() => {
              window.location.reload()
            }, 2000)
    ...
    exportAsXLSX(): void {
      ...
      this.serviceProviderService.post('portfolio/report/read', obj).subscribe(data => {
        let model: any = {};
        let result: any = [];
        model = data;
