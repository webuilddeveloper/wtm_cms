...
export class EmployeeCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('employeeCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('employeeCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['employee-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['employee-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('employee/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('employee/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
