...
@Component({
  ...
  styleUrls: ['./employee-category-edit.component.css']
})
export class EmployeeCategoryEditComponent implements OnInit {
  ...
  create() {
    ...
    this.serviceProviderService.post('employee/category/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('employee/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    // if (this.editModel.image != undefined)
    //   this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('employee/category/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['employee-category'], { skipLocationChange: true });
  }
