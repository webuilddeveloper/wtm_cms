...
@Component({
  selector: 'app-portfolio-category-criteria',
  templateUrl: './portfolio-category-criteria.component.html',
  styleUrls: ['./portfolio-category-criteria.component.css']
})
export class PortfolioCategoryCriteriaComponent implements OnInit {
  ...
  @Input() displayMode: MatAccordionDisplayMode
    ...
    ngOnInit(): void {
      ...
      if (localStorage.getItem('portfolioCategoryPage') != null) {
        let model: any = [];
        model = JSON.parse(localStorage.getItem('portfolioCategoryPage'));
