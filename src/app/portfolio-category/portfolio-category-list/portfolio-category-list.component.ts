...
@Component({
  ...
  styleUrls: ['./portfolio-category-list.component.css']
})
export class PortfolioCategoryListComponent implements OnInit {
  ...
  @Input() messageInput: any = [];
    ...
    ngOnInit(): void {
      ...
      if (localStorage.getItem('portfolioCategoryPage') != null) {
        this.permission = JSON.parse(localStorage.getItem('portfolioCategoryPage'));
      }
    }
    ...
    create() {
      this.router.navigate(['portfolio-category-edit', ''], { skipLocationChange: true });
    }
  
    view(param) {
      this.router.navigate(['portfolio-category-edit', param], { skipLocationChange: true });
    }
    ...
    delete(param, idx) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        if (result) {
          this.serviceProviderService.post('portfolio/category/delete', { code: param.code }).subscribe(data => {
    ...
    deleteAll() {
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        if (result) {
          ...
          this.serviceProviderService.post('portfolio/category/delete', { code: model }).subscribe(data => {
            this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
            setTimeout(() => {
