...
@Component({
  selector: 'app-cms-visitor-list',
  templateUrl: './cms-visitor-list.component.html',
  styleUrls: ['./cms-visitor-list.component.css']
})
export class CmsVisitorListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('cmsvisitorPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('cmsvisitorPage'));
    }
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('ip/read', this.criteriaModel).subscribe(data => {
      ...
      this.excelService.exportAsExcelFile(result, 'รายงานผู้เข้าชมเว็บไซต์_CMS');
    }, err => {
