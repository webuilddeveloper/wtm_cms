...
@Component({
  selector: 'app-cms-visitor',
  templateUrl: './cms-visitor.component.html',
  styleUrls: ['./cms-visitor.component.css']
})
export class CmsVisitorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('cmsvisitorPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('cmsvisitorPage'));
  ...
  read() {
    ...
    if (this.isAdvanceSearch)
      ...
      this.serviceProviderService.post('ip/readCms', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
